...
def get_profiles() -> Dict[str, ProfileSpec]:
    ...
    def _vla_v4_add_args(parser) -> None:
        from . import vla_v4

        vla_v4.add_cli_args(parser)

    def _vla_v4_run(args) -> int:
        from . import vla_v4

        return vla_v4.run(args)
    ...
    return {
        ...
        # vla_v1 + calibrated wrist camera (collect_v4.sh). Thin wrapper, not a fork.
        "vla_v4": ProfileSpec(name="vla_v4", add_cli_args=_vla_v4_add_args, run=_vla_v4_run),
    }
