"""vla_temp: vla_v1 data collection with YCB clutter and three end-of-workspace bins.

Same session layout, planners, logging, and CLI surface as ``vla_v1``, with these defaults:

* Many Isaac Nucleus YCB objects in front of the bins (``--use-ycb`` on by default).
...
def add_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    # vla_temp: YCB clutter in front of the bins (respawn also caps X before the bin zone).
    # (These args are registered by scripts/cli.add_demo_cli_args before this profile hook runs.)
    try:
        parser.set_defaults(
            spawn_min=[0.26, -0.34, 0.89],
            spawn_max=[0.42, 0.36, 1.06],
            min_distance=0.16,
            num_objects=10,
            use_ycb=True,
            spawn_mode="usd",
            ycb_small_only=True,
            respawn_every_n_episodes=1,
        )
    except Exception:
        pass
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="When spawning YCB USDs, only use compact/hand-sized objects (cans, small boxes, fruit, mug, etc.).",
    )
    parser.add_argument(
        "--no-ycb-small-only",
        action="store_false",
        dest="ycb_small_only",
        help="Allow any YCB asset (includes large boxes, pitcher, drill, etc.).",
    )
    parser.add_argument(
        "--objects-include-labels",
        type=str,
        nargs="*",
        default=[],
        help=(
            "Optional explicit YCB label allowlist (overrides --ycb-small-only). "
            "Labels are basenames like 'tomato_soup_can', 'mug'."
        ),
    )
    parser.add_argument(
        "--approach-detour-m",
    ...
    parser.add_argument("--stabilize-steps", type=int, default=60, help="Hold steps before first plan (physics settle)")
    ...
    # Bins (three fixed containers at the far end of the workspace)
    parser.add_argument("--num-bins", type=int, default=3, help="Number of destination bins (default: 3).")
    parser.add_argument(
        "--bin-center-x",
        type=float,
        default=0.78,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.22,
        help="Distance between bin centers along Y (m). Auto-increased if smaller than bin outer width.",
    )
    parser.add_argument(
        "--bin-footprint",
        type=float,
        nargs=2,
        default=[0.14, 0.14],
        help="Bin inner footprint (length along X, width along Y) in m. ~14cm fits typical YCB props.",
    )
    parser.add_argument(
        "--bin-wall-height",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.32,
        help="Cap object spawn/respawn X to bin_center_x minus this (m). Larger = more gap before bins.",
    )
    parser.add_argument(
        "--target-bin-clearance-xy-m",
        type=float,
        default=0.26,
        help="When choosing targets, ignore objects closer than this (XY) to any bin center.",
    )
    parser.add_argument(
        "--transit-clearance-m",
        type=float,
        default=0.22,
        help="EE Z above table during transit over clutter to the bin (m).",
    )
    parser.add_argument(
        "--drop-clearance-m",
        type=float,
        default=0.20,
        help="EE Z above the bin floor when releasing the object (m).",
    )
    parser.add_argument(
        "--hold-after-release-steps",
        type=int,
        default=24,
        help="Hold steps after opening the gripper over the bin.",
    )
    parser.add_argument(
        "--drop-success-margin-m",
        type=float,
        default=0.04,
        help="XY margin (m) when checking whether the object landed inside the bin.",
    )
    parser.add_argument(
        "--no-place-in-bins",
        action="store_true",
        help="If set, end the episode after lift (vla_v1 behavior) instead of transit/drop/release.",
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "suppress_spam", False):
        ...
        print("[VLA_TEMP][WARN] --suppress-spam is currently a no-op (kept for compatibility).")

    import torch  # noqa: E402
    try:
        import numpy as np  # noqa: E402
    except Exception as e:
        print(f"[VLA_TEMP] ERROR: numpy is required for image saving but could not be imported: {e}")
        print("[VLA_TEMP] Please install numpy in your environment (e.g., `pip install numpy`) and retry.")
        return 2
    ...
    print(f"[VLA_TEMP] enable_cameras flag value: {enable_cameras}")
    print(f"[VLA_TEMP] carb /isaaclab/cameras_enabled={carb_settings.get('/isaaclab/cameras_enabled')}")
    ...
    from environments.utils.ycb_labels import YCB_SMALL_LABELS
    ...
    if enable_cameras and DEFAULT_TOP_DOWN_CAMERA is not None:
        ...
        print(f"[VLA_TEMP] Top-down camera created at: {DEFAULT_TOP_DOWN_CAMERA.prim_path}")
    ...
    def _spawn_bins() -> List[Dict[str, object]]:
        """Spawn three open-top static bins using plain USD cubes (no RTX PreviewSurface)."""
        import omni.usd
        ...
        inner_lx, inner_ly = (float(v) for v in getattr(args, "bin_footprint", [0.14, 0.14]))
        wall_h = float(getattr(args, "bin_wall_height", 0.05))
        wall_t = float(getattr(args, "bin_wall_thickness", 0.008))
        base_t = float(getattr(args, "bin_base_thickness", 0.006))
        spacing = float(getattr(args, "bin_y_spacing", 0.22))
        # Keep bins from overlapping along Y (outer width + visible gap between neighbors).
        outer_y = float(inner_ly) + 2.0 * float(wall_t)
        spacing = max(spacing, outer_y + 0.04)

        bin_floor_clearance = 0.005
        base_bottom_z = float(table_z) + bin_floor_clearance
        base_center_z = base_bottom_z + 0.5 * base_t
        bin_floor_z = base_bottom_z + base_t
        ...
        def _add_static_box(
            ...
            px, py, pz = (float(center_xyz[0]), float(center_xyz[1]), float(center_xyz[2]))
            try:
            ...
            if not prim.IsValid():
                return
            # UsdGeom.Cube default edge length is 2.0; set size=1 so scale == meters.
            try:
            ...
            xform.AddTranslateOp(UsdGeom.XformOp.PrecisionDouble).Set(Gf.Vec3d(px, py, pz))
            xform.AddScaleOp(UsdGeom.XformOp.PrecisionDouble).Set(Gf.Vec3d(sx, sy, sz))
            gprim = UsdGeom.Gprim(prim)
            gprim.GetDisplayColorAttr().Set([Gf.Vec3f(float(rgb[0]), float(rgb[1]), float(rgb[2]))])
            try:
        ...
        for i, y in enumerate(y_centers, start=0):
            ...
            _add_static_box(
                base_path,
                (inner_lx + 2 * wall_t, inner_ly + 2 * wall_t, base_t),
                (cx, y, base_center_z),
                rgb,
            )
            ...
            for wname, wsize, wpos in walls:
                ...
                _add_static_box(wpath, wsize, wpos, rgb)
    ...
    try:
        print(f"[VLA_TEMP] Spawned {len(bins_meta)} bins at the far end of the workspace.")
        for _b in bins_meta:
            cx_, cy_ = _b["center_xy"]  # type: ignore[index]
            print(
                f"[VLA_TEMP]   bin {int(_b['idx']) + 1} ({_b['color_name']}): "
    ...
    def _spawn_objects() -> tuple[list[str], Dict[str, str]]:
        ...
        if loader is None:
            ...
            smin = [float(v) for v in getattr(args, "spawn_min", (0.26, -0.34, 0.89))]
            smax = [float(v) for v in getattr(args, "spawn_max", (0.46, 0.36, 1.06))]
            x_cap = float(getattr(args, "bin_center_x", 0.70)) - float(
                getattr(args, "spawn_bin_clearance_m", 0.24)
            )
            smax[0] = min(float(smax[0]), float(x_cap))
            include_labels: list[str] | None = None
            custom_labels = [str(x) for x in (getattr(args, "objects_include_labels", None) or []) if str(x)]
            if custom_labels:
                include_labels = custom_labels
            elif bool(getattr(args, "ycb_small_only", True)) and spawn_mode == "usd":
                include_labels = list(YCB_SMALL_LABELS)
            loader_cfg = ObjectLoaderConfig(
                dataset_dirs=dataset_dirs,
                bounds=SpawnBounds(min_xyz=tuple(smin), max_xyz=tuple(smax)),
                ...
                print_available_labels=bool(include_labels),
                **phys_loader_kwargs,
            )
            loader = ObjectLoader(loader_cfg)
            if include_labels and spawn_mode == "usd":
                try:
                    allow = {x.lower() for x in include_labels}
                    matched = sorted(
                        {str(lbl) for lbl in loader._usd_path_to_label.values() if str(lbl).lower() in allow}
                    )
                    print(f"[VLA_TEMP] YCB small-object pool ({len(matched)} in dataset): {matched}")
                except Exception:
                    pass
    ...
    try:
        print(
            f"[VLA_TEMP] Spawned {len(spawned_paths)} objects "
            f"(spawn_mode={getattr(args, 'spawn_mode', 'usd')!r})."
        )
    except Exception:
        pass
        ...
        if sel in ("farthest", "farthest_no_repeat"):
            if object_pos_b_by_leaf and len(candidates) > 0:
                ...
                if not pool:
                    try:
                        print(
                            "[VLA_TEMP][TARGET] farthest: no objects inside workspace XY reach box "
    ...
    def _select_bin(ep_idx: int) -> Dict[str, object]:
        n_bins = len(bins_meta)
        if n_bins <= 0:
            return {}
        sel = str(getattr(args, "bin_selection", "random"))
        if sel == "fixed":
        ...
        if sel == "random":
            ...
            return _random.Random(int(ep_idx) * 9173 + 1).choice(bins_meta)
    ...
    def _nearest_bin_xy_dist_w(wx: float, wy: float) -> float:
        ...
        for b in bins_meta:
            ...
            dmin = min(dmin, _math.hypot(float(wx) - float(cx), float(wy) - float(cy)))
        return float(dmin)
    ...
    def _make_language_command(
        *,
        ep_idx: int,
        target_prim: str,
        bin_meta: Optional[Dict[str, object]] = None,
    ) -> tuple[str, dict]:
        ...
        bin_color = str((bin_meta or {}).get("color_name", "")) if bin_meta else ""
        bin_idx = int((bin_meta or {}).get("idx", 0)) + 1 if bin_meta else 0
        templates = [
            "Pick up the {label}.",
        ...
        if bin_color:
            templates += [
                "Pick up the {label} and place it in the {bin_color} bin.",
                "Grab the {label} and drop it into the {bin_color} bin.",
                "Move the {label} into the {bin_color} bin.",
                "Pick up the {label} and put it in bin {bin_idx}.",
            ]
        ...
        if bin_meta:
            meta.update(
                {
                    "bin_idx": int(bin_idx),
                    "bin_idx0": int(bin_meta.get("idx", 0)),
                    "bin_color": bin_color,
    ...
    def _rerandomize_object_poses(
        ...
        *,
        ...
        bmin = tuple(float(v) for v in getattr(args, "spawn_min", (0.26, -0.34, 0.89)))
        bmax = list(float(v) for v in getattr(args, "spawn_max", (0.46, 0.36, 1.06)))
        x_cap = float(getattr(args, "bin_center_x", 0.70)) - float(getattr(args, "spawn_bin_clearance_m", 0.24))
        bmax[0] = min(float(bmax[0]), float(x_cap))
        bmax = tuple(bmax)
    ...
    def _create_camera_sensor() -> Optional[Camera]:
        """Create IsaacLab Camera sensor (must exist before first sim.reset())."""
        if not enable_cameras or DEFAULT_TOP_DOWN_CAMERA is None:
            return None
        w = int(DEFAULT_TOP_DOWN_CAMERA.resolution[0])
        h = int(DEFAULT_TOP_DOWN_CAMERA.resolution[1])
        prim_path = str(DEFAULT_TOP_DOWN_CAMERA.prim_path)
        configs = [
            CameraCfg(
                prim_path=prim_path,
            ...
            # Fallback used by vla_lab/eval_isaaclab.py (fewer attach options).
            CameraCfg(
                prim_path=prim_path,
                width=w,
                height=h,
        ...
        for i, camera_cfg in enumerate(configs):
            try:
                cam = Camera(cfg=camera_cfg)
                print(f"[VLA_TEMP] Camera sensor created: {w}x{h} (config variant {i + 1})")
                print(f"[VLA_TEMP] Attached to existing camera prim: {prim_path}")
                return cam
            except Exception as e:
                last_err = e
        print(f"[VLA_TEMP] ERROR: Failed to create Camera object: {last_err}")
        return None

    def _ensure_timeline_playing() -> None:
        """Physics / controller.step require the timeline to be playing."""
        try:
            import omni.timeline

            tl = omni.timeline.get_timeline_interface()
            if not bool(tl.is_playing()):
                tl.play()
        except Exception:
            pass

    # Camera before sim.reset (same order as vla_v1). Must exist before PLAY so SensorBase inits.
    camera_sensor = _create_camera_sensor()
    ...
    try:
        ...
        print(f"[VLA_TEMP] ERROR: sim.reset() failed: {reset_err}")
        if camera_sensor is not None:
            ...
            try:
                ...
                _builtins.ISAACLAB_CALLBACK_EXCEPTION = None
            except Exception:
                pass
            _reset_sim_and_robot()
        else:
    ...
    if camera_sensor is not None:
        try:
            camera_sensor.reset()
            print("[VLA_TEMP] Camera sensor reset OK (post sim.reset)")
        except Exception as e:
            print(f"[VLA_TEMP] WARN: Camera disabled after reset ({e})")
            camera_sensor = None

    print("[VLA_TEMP] Physics scene ready (bins + YCB objects).")
    ...
    if (not getattr(args, "headless", False)) and control_mode == "keyboard":
        ...
        def _after_lift_success(ep_idx: int) -> None:
            """Queue transit/drop waypoints into the episode bin, or end if disabled."""
            vla_planner_state["episode_lift_success"] = True
            if bool(getattr(args, "no_place_in_bins", False)):
                vla_planner_state["stage"] = "done"
                return
            bin_meta = vla_planner_state.get("bin_meta") or {}
            if not bin_meta:
                vla_planner_state["stage"] = "done"
                return
            try:
                ...
                bin_floor_z_w = float(bin_meta.get("bin_floor_z", table_z))
                transit_clear = float(getattr(args, "transit_clearance_m", 0.22))
                drop_clear = float(getattr(args, "drop_clearance_m", 0.20))
                transit_z_b = world_to_base_pos(
                    sim, robot, (float(bcx), float(bcy), float(table_z) + transit_clear)
                )[2]
                drop_floor_z_b = world_to_base_pos(sim, robot, (float(bcx), float(bcy), bin_floor_z_w))[2]
                drop_z_b = float(drop_floor_z_b) + float(drop_clear)
                bin_x_b, bin_y_b, _ = world_to_base_pos(sim, robot, (float(bcx), float(bcy), bin_floor_z_w))
                ee = get_ee_pos_base_frame(robot, str(getattr(args, "ee_link", "j2n6s300_end_effector")))
                transit_z_use = float(transit_z_b)
                if ee is not None:
                    transit_z_use = max(transit_z_use, float(ee[2]))
                pts = [
                    _clamp_waypoint_b((float(bin_x_b), float(bin_y_b), transit_z_use)),
                    _clamp_waypoint_b((float(bin_x_b), float(bin_y_b), float(drop_z_b))),
                ]
                dense = _densify_waypoints_b(
                    pts, max_seg_m=float(getattr(args, "planner_waypoint_max_seg_m", 0.01))
                )
                session_logger.log_event(
                    "action_start",
                    {
                        "action": "TRANSIT_AND_DROP",
                        "episode_idx": int(ep_idx),
                        "bin_idx": int(bin_meta.get("idx", 0)) + 1,
                        "bin_color": bin_meta.get("color_name"),
                        "bin_xy_b": [float(bin_x_b), float(bin_y_b)],
                        "drop_z_b": float(drop_z_b),
                    },
                )
                wp.set_waypoints_b(dense)
                vla_planner_state["place_transit_queued"] = True
                vla_planner_state["stage"] = "place_transit"
                controller.set_mode("translate")
            except Exception as exc:
                print(f"[VLA_TEMP][PLACE] WARN: could not queue bin waypoints: {exc}")
                vla_planner_state["stage"] = "done"

        if loader is None or len(spawned_paths) == 0:
            print("[VLA_TEMP][PLANNER] ERROR: planner control requires spawned objects to grasp.")
            print("[VLA_TEMP][PLANNER] Ensure you run WITHOUT --no-objects and WITH --num-objects >= 1.")
        ...
        print(f"[VLA_TEMP][PLANNER] Control enabled. planner={getattr(args, 'planner', 'scripted')} cfg_dir={cfg_dir}")
        ...
        vla_planner_state = {
            "stage": "init_open",  # init_open -> approach -> close -> lift -> place_transit -> release -> done
            "target_prim": None,
            "lift_pt": None,
    ...
    print(f"[VLA_TEMP] Data collection started!")
    print(f"[VLA_TEMP] Session directory: {session_logger.root}")
    print(f"[VLA_TEMP] Images directory: {images_dir}")
    print(f"[VLA_TEMP] Logging rate: {tick_cfg.log_rate_hz} Hz")
    print(f"[VLA_TEMP] Duration: {getattr(args, 'duration_s', 30.0)} seconds")
    print(f"[VLA_TEMP] Control mode: {getattr(args, 'control', 'keyboard')}")
    print(f"[VLA_TEMP] Camera sensor: {'Active' if camera_sensor is not None else 'Inactive'}")
    ...
    if control_mode != "planner":
        ...
        for ep in range(num_episodes):
            if not simulation_app.is_running():
                print(f"[VLA_TEMP][EP] Simulation stopped, ending at episode {ep}/{num_episodes}")
                break

            print(f"[VLA_TEMP][EP] Starting episode {ep+1}/{num_episodes}")
            ...
            try:
                ...
                if do_respawn:
                    ...
                    if regenerate:
                        print(
                            f"[VLA_TEMP][RESPAWN] ep={int(ep)} regenerate=True respawn_every={int(respawn_every)} n_objects={len(spawned_paths)}"
                        )
                        ...
                        if new_poses is not None:
                            ...
                            try:
                                ...
                                print(f"[VLA_TEMP][RESPAWN] intended_xy={pose_xy}")
                                session_logger.log_event(
                                    "object_respawn",
                                    {
                                        "episode_idx": int(ep),
                            ...
                            try:
                                ...
                                print(f"[VLA_TEMP][RESPAWN] actual_xy={actual_xy}")
                                try:
                                    session_logger.log_event(
                            ...
                            print(
                                "[VLA_TEMP][RESPAWN][WARN] Respawn requested but teleport failed (no poses applied). "
                ...
                episode_bin_meta = _select_bin(int(ep))
                ...
                vla_planner_state.update(
                    {
                        ...
                        "bin_meta": episode_bin_meta,
                        "place_in_bins": not bool(getattr(args, "no_place_in_bins", False)),
                        "place_transit_queued": False,
                        "place_drop_queued": False,
                ...
                try:
                    ...
                    print(
                        f"[VLA_TEMP][EP] start ep={ep} target={tgt} "
                        f"label='{_prim_label(str(tgt or ''))}' n_objects={len(spawned_paths)} "
                        f"candidates={[c.split('/')[-1] for c in candidates]}"
                    )
                except Exception:
                    print(
                        f"[VLA_TEMP][EP] start ep={ep} target={vla_planner_state.get('target_prim')} "
                ...
                while simulation_app.is_running() and steps < max_steps_ep:
                    ...
                    try:
                        ...
                        if stab_left > 0:
                            ...
                            if str(vla_planner_state.get("stage", "idle")) == "idle":
                                if len(spawned_paths) != 0:
                                    ...
                                    if not should_plan:
                                        ...
                                        if not target_prim:
                                            raise RuntimeError("[VLA_TEMP][PLANNER] No target prim available for this episode.")
                                        ...
                                        if ep == 0 and steps < 5:
                                            print(f"[VLA_TEMP][CUROBO] update_world_from_scene ok={ok} n_prims={len(obstacle_prims)}")
                            ...
                            if str(vla_planner_state.get("stage", "")) == "place_transit":
                                controller.set_mode("translate")
                                if (
                                    vla_planner_state.get("place_transit_queued", False)
                                    and len(getattr(wp, "_waypoints_b", [])) == 0
                                ):
                                    session_logger.log_event(
                                        "action_end",
                                        {"action": "TRANSIT_AND_DROP", "episode_idx": int(ep)},
                                    )
                                    vla_planner_state["stage"] = "release"
                                    vla_planner_state["release_left"] = int(
                                        getattr(args, "gripper_open_steps", 10)
                                    )
                                    vla_planner_state["release_queued"] = False

                            if str(vla_planner_state.get("stage", "")) == "release":
                                release_left = int(vla_planner_state.get("release_left", 0))
                                if not vla_planner_state.get("release_queued", False):
                                    session_logger.log_event(
                                        "action_start",
                                        {
                                            "action": "RELEASE_GRIPPER",
                                            "steps": int(release_left),
                                            "episode_idx": int(ep),
                                        },
                                    )
                                    try:
                                        controller.set_mode("gripper")
                                        wp.queue_gripper(+1.0, steps=release_left)
                                    except Exception:
                                        pass
                                    vla_planner_state["release_queued"] = True
                                controller.set_mode("gripper")
                                if release_left > 0:
                                    vla_planner_state["release_left"] = release_left - 1
                                else:
                                    session_logger.log_event(
                                        "action_end",
                                        {"action": "RELEASE_GRIPPER", "episode_idx": int(ep)},
                                    )
                                    vla_planner_state["stage"] = "post_release_hold"
                                    vla_planner_state["post_release_hold_left"] = int(
                                        getattr(args, "hold_after_release_steps", 24)
                                    )
                                    controller.set_mode("translate")

                            if str(vla_planner_state.get("stage", "")) == "post_release_hold":
                                hold_left = int(vla_planner_state.get("post_release_hold_left", 0))
                                controller.set_mode("translate")
                                if hold_left > 0:
                                    vla_planner_state["post_release_hold_left"] = hold_left - 1
                                else:
                                    target_prim = str(vla_planner_state.get("target_prim", ""))
                                    z = _target_z_from_tracker(target_prim) if target_prim else None
                                    bin_meta = vla_planner_state.get("bin_meta") or {}
                                    ...
                                    try:
                                        if z is not None and bin_meta:
                                            ...
                                            for o in tracker.snapshot():
                                                ...
                                                if abs(wx - float(cx)) <= 0.5 * float(lx) + margin:
                                                    if abs(wy - float(cy)) <= 0.5 * float(ly) + margin:
                                                        dropped_ok = True
                                                break
                                    except Exception:
                                        dropped_ok = False
                                    session_logger.log_event(
                                        "drop_result",
                                        {
                                            ...
                                            "ok": bool(dropped_ok),
                                            "target_z": z,
                                            "bin_color": bin_meta.get("color_name") if bin_meta else None,
                                        },
                                    )
                                    vla_planner_state["stage"] = "done"
                            ...
                            if str(vla_planner_state.get("stage", "")) == "done":
                                ...
                                print(f"[VLA_TEMP][EP] end ep={ep} steps={steps} attempts={int(vla_planner_state.get('attempt_idx', 0))} ticks={session_logger.tick_idx} images={images_captured_episode}")
                        ...
                        if session_logger.tick_idx < 10 or steps % 100 == 0:
                            print(f"[VLA_TEMP][PLANNER][WARN] Planner control error at ep={ep} step={steps} tick={session_logger.tick_idx}: {e}")
                ...
                if str(vla_planner_state.get("stage", "")) != "done":
                    ...
                    print(f"[VLA_TEMP][EP] end ep={ep} (truncated) steps={steps} ticks={session_logger.tick_idx} images={images_captured_episode}")
                    # Track totals before closing
                    total_ticks += session_logger.tick_idx
                    total_images += images_captured_episode
                    # Close logger for this episode
                ...
                import traceback
                print(f"[VLA_TEMP][EP][ERROR] Episode {ep} failed with error: {ep_error}")
                traceback.print_exc()
    ...
    print(f"\n[VLA_TEMP] Data collection completed!")
    if control_mode == "planner":
        print(f"[VLA_TEMP] Session directory: {session_folder}")
        print(f"[VLA_TEMP] Total episodes: {num_episodes}")
        print(f"[VLA_TEMP] Total ticks logged: {total_ticks}")
        print(f"[VLA_TEMP] Total images captured: {total_images}")
    else:
        print(f"[VLA_TEMP] Total ticks logged: {session_logger.tick_idx}")
        print(f"[VLA_TEMP] Total images captured: {images_captured}")
