...
#
#   ./vla_lab/scripts/rehab_power.sh
#   N=8,12,16,24,32 SIMS=100 ./vla_lab/scripts/rehab_power.sh
#   EFFECT_SCALE=1.5 ./vla_lab/scripts/rehab_power.sh     # a stronger COACH manipulation (§12.7)
#
...
N="${N:-8,12,16,24}"
SIMS="${SIMS:-40}"
OUT="${OUT:-vla_lab/results/rehab_phase0/power_memo.json}"
...
exec python -m vla_lab.rehab.power --n "${N}" --sims "${SIMS}" --out "${OUT}" "${EXTRA[@]}" "$@"
