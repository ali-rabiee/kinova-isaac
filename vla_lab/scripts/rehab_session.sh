...
#   2. ./vla_lab/scripts/rehab_twin_dryrun.sh passes (M2)
...
#   5. ./vla_lab/scripts/rehab_calibrate.sh has been run FOR THIS PARTICIPANT
#
...
CONFIG="${CONFIG:-vla_lab/configs/rehab_phase0.yaml}"
...
exec python -m vla_lab.rehab.run_session \
  --config "${CONFIG}" \
  --apparatus "${APPARATUS}" \
  --observer "${OBSERVER}" \
