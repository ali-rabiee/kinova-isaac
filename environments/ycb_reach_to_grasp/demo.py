"""Smoke-test demo for the YCB reach-to-grasp environment.

Builds the scene, spawns YCB objects, and runs a Cartesian-velocity jog with
keyboard input + episode logging. Mirrors the previous
``environments/reach_to_grasp/demo.py`` behaviour but routes scene/object
construction through :class:`YCBReachToGraspEnv`.
...
# Pin repo root on sys.path so this script runs both as
# ``python -p .../demo.py`` and as ``-m`` from elsewhere.
...
# Some Isaac Sim startup paths preload a non-package ``environments`` shim
# that breaks ``import environments.<...>``. Evict it so our local package wins.
...
def main():
    parser = argparse.ArgumentParser(description="YCB reach-to-grasp Cartesian jog demo.")
    ...
    env = YCBReachToGraspEnv(device=args_cli.device, scale_range=scale_range)
